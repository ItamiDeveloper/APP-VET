-- =======================================================
-- 🏥 VETERINARIA SAAS - INSTALACIÓN COMPLETA Y DEFINITIVA
...
--   ✅ Datos iniciales completos (especies, razas, roles, categorías)
--   ✅ 2 Tenants de prueba (VET001 y VET002) con suscripciones activas
--   ✅ Usuarios de prueba con contraseñas correctas
--   ✅ Datos de prueba para todas las entidades
--
-- 🔐 CREDENCIALES PREDETERMINADAS:
--   Super Admin:     superadmin / admin123
--   Tenant VET001:   admin / admin123
--   Tenant VET002:   admin / admin123
--
-- ⚙️ INSTRUCCIONES:
--   1. Abre MySQL Workbench o tu cliente MySQL preferido
--   2. Ejecuta este script completo (Ctrl+Shift+Enter)
--   3. Espera a que termine (30-60 segundos)
--   4. Verifica el mensaje de éxito al final
--   5. Inicia tu servidor Spring Boot: .\mvnw.cmd spring-boot:run
--   6. Frontend: npm run dev (puerto 3000)
--   7. Backend: http://localhost:8080
--
-- =======================================================
...
SELECT '🚀 INICIANDO INSTALACIÓN DE BASE DE DATOS...' AS mensaje;

-- ===========================================
-- 📊 NIVEL 1: TABLAS GLOBALES DEL SAAS
-- ===========================================

SELECT '📦 Creando tablas globales del SaaS...' AS paso;
...
-- ===========================================
-- 📊 NIVEL 2: TABLAS DE CATÁLOGOS GLOBALES
-- ===========================================

SELECT '📚 Creando catálogos globales...' AS paso;
...
-- Agregar FK de producto a categoría
...
('superadmin', '$2a$10$xn3LI/AjqicFYZFruSwve.681477XaVNaUQbr1gioaWPn4t1KsnmG', 'superadmin@appvet.com', 'Super', 'Administrador', '999888777', 'ACTIVO');
...
SELECT '🏥 Creando tenants de prueba...' AS paso;
...
INSERT INTO suscripcion (id_tenant, id_plan, fecha_inicio, fecha_fin, metodo_pago, monto_pagado, estado, notas) VALUES
((SELECT id_tenant FROM tenant WHERE codigo_tenant = 'VET001'), 2, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 1 MONTH), 'TARJETA', 99.99, 'ACTIVO', 'Suscripción mensual - Plan Profesional'),
((SELECT id_tenant FROM tenant WHERE codigo_tenant = 'VET002'), 2, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 1 MONTH), 'TARJETA', 99.99, 'ACTIVO', 'Suscripción mensual - Plan Profesional');

SELECT '✅ Tenants de prueba creados' AS resultado;

-- ===========================================
-- 👥 USUARIOS Y DATOS DE PRUEBA
-- ===========================================

SELECT '👥 Creando usuarios y datos de prueba...' AS paso;

-- Variables para IDs de tenants
...
-- USUARIOS PARA VET001
-- Password: admin123
INSERT INTO usuario (id_tenant, id_rol, username, password_hash, email, nombres, apellidos, telefono, estado) VALUES
(@tenant1_id, @rol_admin, 'admin', '$2a$10$xn3LI/AjqicFYZFruSwve.681477XaVNaUQbr1gioaWPn4t1KsnmG', 'admin@patitasfelices.com', 'Carlos', 'Rodríguez', '987654321', 'ACTIVO'),
(@tenant1_id, @rol_vet, 'drjuan', '$2a$10$xn3LI/AjqicFYZFruSwve.681477XaVNaUQbr1gioaWPn4t1KsnmG', 'drjuan@patitasfelices.com', 'Juan', 'Pérez', '987654322', 'ACTIVO');

-- USUARIOS PARA VET002
INSERT INTO usuario (id_tenant, id_rol, username, password_hash, email, nombres, apellidos, telefono, estado) VALUES
(@tenant2_id, @rol_admin, 'admin', '$2a$10$xn3LI/AjqicFYZFruSwve.681477XaVNaUQbr1gioaWPn4t1KsnmG', 'admin@amigospeludos.com', 'María', 'González', '987654323', 'ACTIVO'),
(@tenant2_id, @rol_vet, 'drana', '$2a$10$xn3LI/AjqicFYZFruSwve.681477XaVNaUQbr1gioaWPn4t1KsnmG', 'drana@amigospeludos.com', 'Ana', 'Martínez', '987654324', 'ACTIVO');

-- DOCTORES PARA VET001
...
SELECT '📊 RESUMEN DE DATOS CREADOS' AS seccion;
SELECT CONCAT('✓ ', COUNT(*), ' Planes de suscripción') AS detalle FROM plan
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Tenants registrados') FROM tenant
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Suscripciones activas') FROM suscripcion
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Usuarios creados') FROM usuario
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Doctores registrados') FROM doctor
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Clientes registrados') FROM cliente
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Mascotas registradas') FROM mascota
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Citas programadas') FROM cita
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Historias clínicas') FROM historia_clinica
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Productos en catálogo') FROM producto
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Items de inventario') FROM inventario
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Ventas registradas') FROM venta
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Compras registradas') FROM compra;

SELECT '' AS separador;
SELECT '🔐 CREDENCIALES DE ACCESO' AS seccion;
SELECT 'Super Admin: superadmin / admin123' AS credencial
UNION ALL SELECT 'VET001 Admin: admin / admin123'
UNION ALL SELECT 'VET002 Admin: admin / admin123';

SELECT '' AS separador;
SELECT '🚀 PRÓXIMOS PASOS' AS seccion;
SELECT '1. Verifica que MySQL esté corriendo' AS paso
UNION ALL SELECT '2. Inicia el backend: cd app && .\\mvnw.cmd spring-boot:run'
UNION ALL SELECT '3. Inicia el frontend: cd nx-vet && npm run dev'
UNION ALL SELECT '4. Backend: http://localhost:8080'
UNION ALL SELECT '5. Frontend: http://localhost:3000'
UNION ALL SELECT '6. Inicia sesión con las credenciales arriba';
