...
-- 2. Ejecuta ESTE ARCHIVO COMPLETO (todo el script)
-- 3. Espera a que termine (tarda ~10 segundos)
-- 4. ¡Listo! Tu base de datos estará completamente configurada
-- 
-- ⚠️ ADVERTENCIA: Este script eliminará la base de datos 
--    'veterinaria_saas' si existe y la creará desde cero.
-- 
...
-- 
--    Veterinarios:
--    - Usuario: drjuan (VET001) - Contraseña: admin123
--    - Usuario: drana (VET002) - Contraseña: admin123
-- 
-- 🔧 CONFIGURACIÓN DEL BACKEND:
-- 
--    En application.properties debe tener:
...
--    Body: {"username":"admin_vet1","password":"admin123"}
-- 
-- 🌐 SWAGGER UI:
...
SELECT '🚀 INICIANDO INSTALACIÓN...' AS mensaje;
...
-- ✅ GENERADO Y VERIFICADO CON BCryptPasswordEncoder
...
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Tenants (Veterinarias)') FROM tenant
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Usuarios del sistema') FROM usuario
...
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Especies disponibles') FROM especie
UNION ALL SELECT CONCAT('✓ ', COUNT(*), ' Razas disponibles') FROM raza;

SELECT '' AS separador;
SELECT '🔐 CREDENCIALES DE ACCESO' AS seccion;
SELECT '─────────────────────────────────────────────────────' AS separador
UNION ALL SELECT '🔹 SUPER ADMINISTRADOR:'
UNION ALL SELECT '   Usuario: superadmin'
UNION ALL SELECT '   Contraseña: admin123'
UNION ALL SELECT ''
UNION ALL SELECT '🔹 VETERINARIA 1 (VET001 - Patitas Felices):'
UNION ALL SELECT '   Usuario Admin: admin_vet1'
UNION ALL SELECT '   Usuario Doctor: drjuan'
UNION ALL SELECT '   Contraseña para ambos: admin123'
UNION ALL SELECT ''
UNION ALL SELECT '🔹 VETERINARIA 2 (VET002 - Amigos Peludos):'
UNION ALL SELECT '   Usuario Admin: admin_vet2'
UNION ALL SELECT '   Usuario Doctor: drana'
UNION ALL SELECT '   Contraseña para ambos: admin123';

SELECT '' AS separador;
SELECT '🚀 SIGUIENTES PASOS' AS titulo;
SELECT '─────────────────────────────────────────────────────' AS separador
UNION ALL SELECT '1. Verificar application.properties del backend:'
UNION ALL SELECT '   spring.datasource.url=jdbc:mysql://localhost:3306/veterinaria_saas'
UNION ALL SELECT ''
UNION ALL SELECT '2. Iniciar el backend Spring Boot:'
UNION ALL SELECT '   cd app'
UNION ALL SELECT '   ./mvnw spring-boot:run'
UNION ALL SELECT ''
UNION ALL SELECT '3. Probar el login con:'
UNION ALL SELECT '   POST http://localhost:8080/api/auth/login'
UNION ALL SELECT '   Body: {"username":"admin_vet1","password":"admin123"}'
UNION ALL SELECT ''
UNION ALL SELECT '4. Acceder a Swagger UI:'
UNION ALL SELECT '   http://localhost:8080/swagger-ui/index.html';
